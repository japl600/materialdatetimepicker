...
import com.japl600.materialdatetimepicker.date.MonthAdapter.CalendarDay;
...
public abstract class MonthView extends View {
  ...
  private final Calendar mPersianCalendar;
  ...
  public MonthView(Context context, AttributeSet attr, DatePickerController controller) {
    ...
    mPersianCalendar = Calendar.getInstance();
  ...
  public void setMonthParams(HashMap<String, Integer> params) {
    ...
    mPersianCalendar.set(mYear, mMonth, 1);
  ...
  private String getMonthAndYearString() {
    ...
    return mPersianCalendar.getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.getDefault())
...
//    float firstX = (2 * (mNumDays - 1) + 1) * dayWidthHalf + rightSpace; //Quitando orden inverso de días
    float firstX = mEdgePadding;
...
//    int column = (int) (mNumDays - ((x - dayStart) * mNumDays) / (mWidth - dayStart - mEdgePadding));
    int column = (int) (((x - dayStart) * mNumDays) / (mWidth - dayStart - mEdgePadding)); //Quitando orden inverso de días
  ...
  protected class MonthViewTouchHelper extends ExploreByTouchHelper {
    ...
    protected CharSequence getItemDescription(int day) {
      ...
      final String date = SDF.format(mTempCalendar.getTime());
